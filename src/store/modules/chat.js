...
export default {
  ...
  mutations: {
    ...
    SET_MSG_LIST_PUSH: (state, data) => {
      if (!state.msgList.length) {
        ...
        state.msgList[state.msgList.length - 1].msgList.push(data);
      }
    },
  ...
  actions: {
    ...
    set_instance: ({ state, commit, dispatch }, vue) => {
      let host = "ws://192.168.2.82:9999/api";
