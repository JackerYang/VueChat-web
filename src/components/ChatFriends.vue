<template>
  <div class="chat-friends">
    <!-- 最近消息 -->
    <div class="lists" v-if="!showSearch && currentTab === '聊天'">
      <div
        v-for="f in chatList"
        :key="f.id"
      ...
      >
        <Badge :count="f.unReadNum" :offset="[5,5]">
          <img :class="{gray: f.status === 0}" :src="f.avatarUrl" />
        </Badge>
        <div class="right-info">
          <p class="top">
            ...
            <span class="time">{{ f.lastTimeStr }}</span>
          </p>
          <p class="msg">
    ...
    <!-- 好友列表 -->
    <div class="lists" v-if="!showSearch && currentTab === '好友'">
      <div
    ...
    <div class="tabs" v-if="!showSearch">
      <span
        :class="[currentTab === item.title ? 'on' : '']"
      ...
      >
        <Badge
          :count="item.title === '聊天' ? msgCountChat : 0"
          :offset="[10,-4]"
        >
          <Icon :type="item.icon" />
          {{ item.title }}
