<template>
  <div class="chat-message" id="message-list">
    ...
    <div
      ...
      v-if="item.msgList.filter(i => !i.isDelete).length"
      :key="item.msgTime"
    >
      <div class="time">
        <Divider>
          <span>{{ item.msgTimeStr }}</span>
        </Divider>
      </div>
      <!-- 列表 -->
      <div class="message" v-for="msg in item.msgList" v-if="!msg.isDelete" :key="msg.id">
        <!-- 撤回消息 -->
  ...
  methods: {
    ...
    viewMore() {
      ...
      this.$store.dispatch("send_msg", {
        ...
        msgTimeLine: this.msgList[0].msgTime
      });
    },
